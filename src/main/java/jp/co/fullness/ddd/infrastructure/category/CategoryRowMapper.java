...
/**
 * MyBatis の {@link ProductCategoryRow} をドメインの {@link Category} に変換する腐敗防止層（ACL）。
 *
 * <p>DB 由来の Row を、値の検証を VO（{@link CategoryId} / {@link CategoryName}）へ
 * 委ねてドメインの語彙に翻訳する。この層は「変換」に徹する。</p>
 */
@Mapper(componentModel = "spring")
public interface CategoryRowMapper extends ToDomainMapper<ProductCategoryRow, Category> {

    @Override
    default Category toDomain(ProductCategoryRow row) {
        if (row == null) {
